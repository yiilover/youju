...
////嘉易修改部分查询楼盘
	...
	//
$house=$db->pre.'newhouse';
$csql=mysql_query("select itemid,title from  $house where  username='$_username' and isnew=1");
while($class=mysql_fetch_array($csql)){
	$cinfo[]=$class;
...
switch($action) {
	case 'add':
		if($MG['info_limit'] && $limit_used >= $MG['info_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		if($MG['day_limit']) {
		...
		if($submit) {
			...
			if($itemid) {
				...
				$totime = '';
				if($_userid) {
					$user = $db->get_one("SELECT m.truename,m.qq,m.msn,c.areaid,c.mail,c.telephone,c.fax,c.address FROM {$AJ_PRE}member m,{$AJ_PRE}company c WHERE m.userid=c.userid AND m.userid='$_userid'");
					extract($user);
				}
			}
			$item = array();
