...
if($mid > 4 && isset($MODULE[$mid]) && !$MODULE[$mid]['islink']) {
	...
	if($MOD['feed_enable']) {
		...
		if($MOD['feed_enable'] == 2) {
			if($kw) $rss_title = $rss_title.$AJ['seo_delimiter'].$kw;
			if($keyword) $condition .= " and keyword LIKE '%$keyword%'";
			if($catid) {
				$cat .= "&catid=$catid";
				$rss_title = $rss_title.$AJ['seo_delimiter'].strip_tags(cat_pos($catid, $AJ['seo_delimiter']));
			}
			if($areaid) {
				$AREA = cache_read('area.php');
				$condition .= ($AREA[$areaid]['child']) ? " and areaid IN (".$AREA[$areaid]['arrchildid'].")" : " and areaid=$areaid";
				$rss_title = $rss_title.$AJ['seo_delimiter'].strip_tags(area_pos($areaid, $AJ['seo_delimiter']));
			}
		}
	}
	$rss_title = $rss_title.$AJ['seo_delimiter'].$AJ['sitename'];
	$rss_link = AJ_PATH;
	header("content-type:application/xml");
	echo '<?xml version="1.0" encoding="'.AJ_CHARSET.'"?>';
	echo '<rss version="2.0">';
	echo '<channel>';
	echo '<title>'.$rss_title.'</title>';
	echo '<link>'.$rss_link.'</link>';
	echo '<pubDate>'.timetodate($AJ_TIME).'</pubDate>';	
	if($MOD['feed_enable']) {
		...
		echo '<link>'.AJ_PATH.'</link>';
		echo '<description><![CDATA['.$L['rss_close'].']]></description>';
		echo '<pubDate>'.timetodate($AJ_TIME, 6).'</pubDate>';
		echo '</item>';
	}
	echo '</channel>';
	echo '</rss>';
