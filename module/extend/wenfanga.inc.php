	...
	default:
		...
		if($submit) {
			...
			if($status == 3) {
				...
				if($MOD['wenfang_show']) {
					$forward = $MOD['linkurl'].'wenfang.php?mid='.$mid.'&itemid='.$itemid.'&page='.$page.'&rand='.mt_rand(10, 99).'#last';
					dalert('', '', 'parent.window.location="'.$forward.'";');
				} else {
					$forward = $MOD['wenfang_url'].rewrite('index.php?mid='.$mid.'&itemid='.$itemid.'&page='.$page.'&rand='.mt_rand(10, 99)).'#last';
			...
			$template = 'wenfanga';
			include template('wenfanga', $module);
		}
	break;
}
?>