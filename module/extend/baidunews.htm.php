...
if(!$MOD['baidunews']) {
	file_del(AJ_ROOT.'/baidunews.xml');
	return false;
}
$news = array();
foreach($MODULE as $m) {
...
$data = '<?xml version="1.0" encoding="'.AJ_CHARSET.'"?>';
$data .= '<document>';
$data .= '<webSite>'.AJ_PATH.'</webSite>';
$data .= '<webMaster>'.$MOD['baidunews_email'].'</webMaster>';
$data .= '<updatePeri>'.$MOD['baidunews_update'].'</updatePeri>';
foreach($news as $v) {
	$mid = $v['moduleid'];
	$url = linkurl($v['linkurl']);
	$result = $db->query("SELECT * FROM {$AJ_PRE}article_{$mid} a,{$AJ_PRE}article_data_{$mid} d WHERE a.itemid=d.itemid ORDER BY a.addtime DESC LIMIT $news_num");
...
$data .= '</document>';
$data = str_replace('><', ">\n<", $data);
file_put(AJ_ROOT.'/baidunews.xml', $data);
return true;
?>