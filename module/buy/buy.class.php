...
class buy {
    ...
    function buy($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','areaid','typeid','level','title','style','fee','introduce','amount','price','standard','pack','days','thumb','thumb1','thumb2','tag','status','hits','username','totime','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','note','company','truename','telephone','mobile','address','email','msn','qq','ali','skype','room','hall','balcony','houseearm');
    }
	...
	function update($itemid, $item = array(), $content = '') {
		...
		if($item['username']) {
			...
			if($m) $sql .= ",groupid='$m[groupid]',company='$m[company]',vip='$m[vip]',validated='$m[validated]',truename='$m[truename]',mobile='$m[mobile]',address='$m[address]',email='$m[mail]',msn='$m[msn]',qq='$m[qq]',ali='$m[ali]',skype='$m[skype]'";
		}*/
	...
	function refresh($itemid) {
		...
		$editdate = timetodate($AJ_TIME, 3);
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		$this->db->query("UPDATE {$this->table} SET edittime='$AJ_TIME',editdate='$editdate' WHERE itemid IN ($itemids)");
	}

	function _update($username) {
		global $AJ_TIME;
		$this->db->query("UPDATE {$this->table} SET status=4 WHERE status=3 AND totime>0 AND totime<$AJ_TIME AND username='$username'");
	}
