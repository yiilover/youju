...
$result = $db->query("SELECT * FROM $biao WHERE title like '%".$q."%' or letter like '%".$q."%'");
...
if($action == 'getnewhouseList'){
$result = $db->query("SELECT * FROM $biao WHERE isnew='1' and (title like '%".$q."%' or letter like '%".$q."%')");
...
foreach ($rowss as $key=>$value) {
	...
	$str .= "没有你所查询的楼盘|addBorough|addBorough\n";
		echo $str;
