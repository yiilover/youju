...
$MG['newhouse_limit'] > -1 or dalert(lang('message->without_permission_and_upgrade'), 'goback');
...
if(in_array($action, array('', 'add'))) {
	...
	$limit_used = $r['num'];
	$limit_free = $MG['newhouse_limit'] > $limit_used ? $MG['newhouse_limit'] - $limit_used : 0;
}
if(check_group($_groupid, $MOD['group_refresh'])) $MOD['credit_refresh'] = 0;

switch($action) {
	case 'add':
		if($MG['newhouse_limit'] && $limit_used >= $MG['newhouse_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		if($MG['day_limit']) {
		...
		if($MG['newhouse_free_limit'] >= 0) {
			$fee_add = ($MOD['fee_add'] && (!$MOD['fee_mode'] || !$MG['fee_mode']) && $limit_used >= $MG['newhouse_free_limit'] && $_userid) ? dround($MOD['fee_add']) : 0;
		} else {
			$fee_add = 0;
		}
	...
	default:
		...
		$condition = "username='$_username' AND status=$status and isnew=1";
