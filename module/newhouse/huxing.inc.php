...
if($item && $item['status'] > 2) {
	if($MOD['huxing_html'] && is_file(AJ_ROOT.'/'.$MOD['moduledir'].'/'.$item['linkurl'])) d301($MOD['linkurl'].$item['linkurl']);
	extract($item);
} else {
...
$r = $db->get_one("SELECT COUNT(*) AS num FROM {$db->pre}photo_12 WHERE houseid=$itemid {$condition}");
$zong=$r['num'];
