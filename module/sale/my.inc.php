...
require MD_ROOT.'/sale.class.php';
...
switch($action) {
	case 'add':
		if($MG['sell_limit'] && $limit_used >= $MG['sell_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		if($MG['day_limit']) {
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if(!$IMVIP && $MG['uploadpt']) $post['thumb1'] = $post['thumb2'] =$post['thumb3'] = $post['thumb4']= '';
				...
				if($_userid) {
					...
					$forward = 'parent.window.location';
				}
		...
		if($submit) {
			if($do->pass($post)) {
				...
				if(!$IMVIP && $MG['uploadpt']) {
					...
					$post['thumb3'] = $item['thumb3'];
					$post['thumb4'] = $item['thumb4'];
				}
				$need_check =  $MOD['check_add'] == 2 ? $MG['check'] : $MOD['check_add'];
