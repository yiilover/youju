...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	$sfields = array('模糊', '小区名',  '标题', '简介', '公司名', '联系人', '联系电话', '联系地址', '电子邮件', '联系MSN', '联系QQ', '会员名', 'IP');
	...
	$sorder  = array('结果排序方式', '更新时间降序', '更新时间升序', '添加时间降序', '添加时间升序', VIP.'级别降序', VIP.'级别升序', '楼盘单价降序', '楼盘单价升序',  '浏览次数降序', '浏览次数升序', '信息ID降序', '信息ID升序');
...
switch($action) {
	...
	break;
	default:
		$lists = $do->get_list('status=3 and isnew=0'.$condition, $dorder[$order]);
		$menuid = 1;
		include tpl('index', $module);
	break;
}
