...
if($itemid) {
	...
	if($action == 'buy' && $r['fee']) {
		...
		if($submit) {			
			...
			$c = $db->get_one("SELECT skin FROM {$AJ_PRE}company WHERE userid=$_userid");
			$c['skin'] or $c['skin'] = 'default';
			$o = $db->get_one("SELECT itemid FROM {$AJ_PRE}style WHERE skin='$c[skin]'");
	...
	if($action == 'view') {
		...
		$c = $db->get_one("SELECT skin,linkurl,domain,styletime FROM {$AJ_PRE}company WHERE userid=$_userid");
		if(!$c['skin']) {
			if($MG['styleid']) {
		...
		$c['skin'] or $c['skin'] = 'default';
		$havedays = $c['styletime'] ? ceil(($c['styletime']-$AJ_TIME)/86400) : 0;
