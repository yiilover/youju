...
if($action == 'add') {
	...
	if($kw && in_array($mid, array(4, 5, 6))) {
		...
		$r = $db->get_one("SELECT * FROM {$AJ_PRE}spread WHERE username='$_username' AND mid=$mid AND word='$kw' AND fromtime>=$next_time");
		if($r) message($L['spread_msg_buy'], $EXT['spread_url']);
		isset($mid) or $mid = 5;
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}spread WHERE mid=$mid AND status=3 AND word='$kw' AND fromtime>=$next_time");
		...
		if($submit) {
			...
			$db->query("INSERT INTO {$AJ_PRE}spread (mid,tid,word,price,currency,company,username,addtime,fromtime,totime,status) VALUES ('$mid','$buy_tid','$kw','$buy_price','$currency','$_company','$_username','$AJ_TIME','$next_time','$totime','$status')");
