...
class member {
	...
	function set_member($member) {
		...
		$member['letter'] = GetPinyins($member['company'],1);
		if($this->userid) {		
	...
	function login($login_username, $login_password, $login_cookietime = 0, $admin = false) {
		...
		$auth = encrypt($user['userid']."\t".$user['username']."\t".$user['groupid']."\t".$user['password']."\t".$user['admin'], md5(AJ_KEY.$_SERVER['HTTP_USER_AGENT']));
	...
	function deluser($table, $user, $name = true, $data = false, $moduleid = 0) {
		...
		if($data) {
			...
			while($r = $this->db->fetch_array($result)) {
				...
				if($moduleid == 5) {
					$this->db->query("DELETE FROM {$AJ_PRE}sale_search_5 WHERE itemid=$itemid");
				}
	...
	function rename($cusername, $nusername) {
		...
		foreach($MODULE as $m) {
			if($m['islink'] || $m['moduleid'] < 5) continue;
			$tables[] = in_array($m['module'], array('article', 'newhouse', 'buy', 'rent', 'info', 'photo', 'sale', 'video')) ? $m['module'].'_'.$m['moduleid'] : $m['module'];
		}
 ...
 */
$pinying = Array();
function GetPinyins($str,$ishead=0,$isclose=1)
{
	global $pinying;
	$restr = '';
	...
	if(count($pinying)==0)
	{
		...
		{
			$line = trim(fgets($fp));
			$pinying[$line[0].$line[1]] = substr($line,3,strlen($line)-3);
		}
		fclose($fp);
	}
	for($i=0;$i<$slen;$i++)
