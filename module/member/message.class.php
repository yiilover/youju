...
class message {
    ...
    function message()	{
		global $db, $AJ_PRE, $_userid, $_username;
		$this->userid = $_userid;
	...
	function send($message) {
		global $AJ, $MODULE, $MOD, $AJ_TIME, $AJ_IP, $_email, $L;
		if(!$this->is_message($message)) return false;
		$message['title'] = htmlspecialchars($message['title']);
		if(isset($message['save'])) {
			$this->db->query("INSERT INTO {$this->pre}message(title,typeid,content,fromuser,touser,addtime,ip,status) values('$message[title]','$message[typeid]','$message[content]','$this->username','$message[touser]','$AJ_TIME','$AJ_IP','1')");
		} else {
			...
			foreach(explode(' ', $message['touser']) as $touser) {
				...
				if($user) {
					...
					if(!in_array($this->username, $blacks) && !in_array($touser, $tousers)) {
						$message['content'] = addslashes(save_local(stripslashes($message['content'])));
						$tousers[] = $touser;
						if(isset($message['copy'])) $this->db->query("INSERT INTO {$this->pre}message (title,typeid,content,fromuser,touser,addtime,ip,feedback,status) VALUES ('$message[title]','$message[typeid]','$message[content]','$this->username','$touser','$AJ_TIME','$AJ_IP','$feedback','2')");
						$this->db->query("UPDATE {$this->pre}member SET message=message+1 WHERE username='$touser'");
						$this->db->query("INSERT INTO {$this->pre}message (title,typeid,content,fromuser,touser,addtime,ip,feedback,status) VALUES ('$message[title]','$message[typeid]','$message[content]','$this->username','$touser','$AJ_TIME','$AJ_IP','$feedback','3')");
					}
				}
			}
		}
	...
	function export($message) {
		...
		if($data) {
			...
			$data = '<html><meta http-equiv="Content-Type" content="text/html;charset='.AJ_CHARSET.'"/><title>'.$this->username.' '.$names[$status].''.$AJ['sitename'].' '.timetodate($AJ_TIME, 5).' - Powered By AIJIACMS.COM</title><style>*{font-size:13px;font-family:Verdana,Arial;}body{width:750px;margin:auto;line-height:200%;}</style><base target="_blank"/><base href="'.AJ_PATH.'"/><body><br/>'.$data.'<a href="http://www.aijiacms.com"><small>Powered By AIJIACMS.COM</small></a><br/></body></html>';
			ob_start();
	...
	function _send($message) {
		...
		if($message['type']) {
			...
			$this->db->query("INSERT INTO {$this->pre}message(title,content,fromuser,touser,addtime,status,groupids) values('$message[title]','$message[content]','$this->username','','$AJ_TIME','0','$message[groupids]')");
		} else {
			foreach(explode(' ', $message['touser']) as $touser) {
