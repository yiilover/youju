...
function make_auth($username) {
	global $AJ_IP;
	return strtoupper(md5(md5($username.'|'.AJ_KEY.'|'.$AJ_IP)));
}

function check_auth($auth) {
	global $db, $L;
	preg_match("/^[A-Z0-9]{32}$/", $auth) or dalert($L['check_auth'], AJ_PATH);
...
function auth_time($time) {
	global $MOD, $AJ_TIME, $L;
	if($MOD['auth_days'] && $AJ_TIME - $time > $MOD['auth_days']*86400) dalert($L['auth_time'], $MOD['linkurl']);
}
?>