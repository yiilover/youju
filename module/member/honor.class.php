...
class honor {
	...
	function pass($post) {
		...
		if(strtotime($post['fromtime'].' 00:00:00') > $AJ_TIME) return $this->_($L['honor_pass_fromdate_error']);
		if($post['totime']) {
			if(!is_date($post['totime'])) return $this->_($L['honor_pass_todate']);
			if(strtotime($post['totime'].' 23:59:59') < $AJ_TIME) return $this->_($L['honor_pass_todate_error']);
		}
		return true;
	}
	...
	function reject($itemid) {
		...
		if(is_array($itemid)) {
			...
			$this->db->query("UPDATE {$this->table} SET status=1,editor='$_username',edittime=$AJ_TIME WHERE itemid=$itemid");
			return true;
		}
	}
