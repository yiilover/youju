...
switch($action) {
	...
	case 'edit':
		$itemid or message();
		$r = $db->get_one("SELECT * FROM {$AJ_PRE}ask WHERE itemid=$itemid AND username='$_username'");
		$r or message();
		...
		if($submit) {
			...
			$sql = substr($sql, 1);
			$db->query("UPDATE {$AJ_PRE}ask SET $sql WHERE itemid=$itemid AND username='$_username' ");
	...
	case 'star':
		...
		$db->query("UPDATE {$AJ_PRE}ask SET star=$star WHERE star=0 and username='$_username' AND itemid=$itemid");
	...
	default:
		...
		$asks = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}ask WHERE username='$_username' $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
