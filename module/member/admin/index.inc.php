...
if(in_array($action, array('', 'check'))) {
	$sfields = array('按条件', '公司名', '会员名', '通行证名','姓名', '手机号码', '部门', '职位', 'Email', 'QQ', 'MSN', '阿里旺旺', 'Skype', '注册IP', '登录IP', '客服专员', '开户银行', '银行帐号', $AJ['trade_nm'], '推荐人');
	...
	$sorder  = array('结果排序方式', '注册时间降序', '注册时间升序', '登录时间降序', '登录时间升序', '登录次数降序', '登录次数升序', '账户'.$AJ['money_name'].'降序', '账户'.$AJ['money_name'].'升序', '会员'.$AJ['credit_name'].'降序', '会员'.$AJ['credit_name'].'升序', '短信余额降序', '短信余额升序');
...
switch($action) {
	...
	case 'login':
		if($userid) {
			...
			msg('授权成功，正在转入会员会员中心...', $MODULE[2]['linkurl'].'?tm='.$AJ_TIME);
		} else {
			msg();
		}
	break;
