...
class know {
	...
	function set($post) {
		...
		if($this->itemid) {
			...
			$post['aid'] = 0;
			$post['totime'] = $AJ_TIME + $MOD['overdays']*86400;
			$post['process'] = 1;
			$post['ip'] = $AJ_IP;
		}
		if(!defined('AJ_ADMIN')) {
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				$this->db->query("DELETE FROM {$AJ_PRE}know_answer WHERE qid=$itemid");
				$this->db->query("DELETE FROM {$AJ_PRE}know_vote WHERE qid=$itemid");
				$content_table = content_table($this->moduleid, $this->itemid, $this->split, $this->table_data);
	...
	function active($itemid) {
		...
		if(is_array($itemid)) {
			...
			$totime = $AJ_TIME + $MOD['overdays']*86400;
			$this->db->query("UPDATE {$this->table} SET totime=$totime,process=1,updatetime=$AJ_TIME WHERE itemid=$itemid AND process=0");
			return true;
		}
	}
