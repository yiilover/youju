...
if(in_array($action, array('', 'check', 'reject', 'recycle'))) {
	...
	$sorder  = array('结果排序方式', '添加时间降序', '添加时间升序', '更新时间降序', '更新时间升序', '浏览次数降序', '浏览次数升序', '答案数量降序', '答案数量升序', '悬赏'.$AJ['credit_name'].'降序', '悬赏'.$AJ['credit_name'].'升序', '信息ID降序', '信息ID升序');
			...
			if($do->pass($post)) {
				...
				if($post['aid'] != $post['nid']) {
					if($post['nid']) {
						...
						if($t) {
							$db->query("UPDATE {$table} SET process=3,updatetime=$AJ_TIME,aid=$aid WHERE itemid=$itemid");
						} else {
							msg('答案ID不属于此问题');
						}
					} else {
						$db->query("UPDATE {$table} SET process=1,updatetime=$AJ_TIME,aid=0 WHERE itemid=$itemid");
					}
				}
				dmsg('修改成功', $forward);
			} else {
