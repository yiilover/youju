...
if($action == 'best') {
	...
	set_cookie('best_answer_'.$itemid, 1, $AJ_TIME + 365*86400);
	exit('1');
}
$item or exit;
include load('misc.lang');
$linkurl = linkurl($MOD['linkurl'].$item['linkurl'], 1);
$table_answer = $AJ_PRE.'know_answer';
$table_vote = $AJ_PRE.'know_vote';
...
switch($action) {
	...
	case 'vote':
		...
		if($could_vote) {
			$totime = $AJ_TIME + $MOD['votedays']*86400;
	...
	case 'vote_add':
		...
		if($could_vote) {
			if($_userid) {
				...
				$v = $db->get_one("SELECT itemid FROM {$table_vote} WHERE qid=$itemid AND ip='$AJ_IP' AND addtime>$AJ_TIME-86400");
			}
		}
		if($v) $could_vote = false;
		set_cookie('answer_vote_'.$itemid, 1, $AJ_TIME + 365*86400);
		if($could_vote) {
			$db->query("INSERT INTO {$table_vote} (qid,aid,username,addtime,ip) VALUES ('$itemid','$aid','$_username','$AJ_TIME','$AJ_IP')");
			$db->query("UPDATE {$table_answer} SET vote=vote+1 WHERE itemid=$aid");
			if($MOD['credit_vote'] && $_username) {
				$could_credit = true;
				if($MOD['credit_maxvote'] > 0) {					
					$r = $db->get_one("SELECT SUM(amount) AS total FROM {$AJ_PRE}finance_credit WHERE username='$_username' AND addtime>$AJ_TIME-86400  AND reason='".$L['vote_answer']."'");
	...
	case 'choose':
		...
		if($could_choose) {
			...
			if($a) {
				...
				$db->query("UPDATE {$table} SET process=3,aid=$aid,comment='$content',updatetime='$AJ_TIME' WHERE itemid=$itemid");
				if($a['username']) {
					if($item['credit']) {
	...
	case 'raise':
		...
		if($could_raise) {
			if($credit >= $MOD['raisecredit']) {
				$addtime = $AJ_TIME;
				$totime = $AJ_TIME + $MOD['overdays']*86400 + $MOD['raisedays']*86400;
			} else {
	...
	default:
		...
		if($submit && $could_answer) {
			...
			$db->query("INSERT INTO {$table_answer} (qid,linkurl,content,username,addtime,ip,status,hidden) VALUES ('$itemid','$url','$content','$_username','$AJ_TIME','$AJ_IP','$status','$hidden')");
			if($MOD['credit_answer'] && $_username && $status == 3) {
				$could_credit = true;
				if($MOD['credit_maxanswer'] > 0) {					
					$r = $db->get_one("SELECT SUM(amount) AS total FROM {$AJ_PRE}finance_credit WHERE username='$_username' AND addtime>$AJ_TIME-86400  AND reason='".$L['answer_question']."'");
			...
			if($item['process'] == 1 && $item['username'] && !$item['message'] && $MOD['messagedays']) {
				if($item['totime'] - $AJ_TIME < $MOD['messagedays']*86400) {
			...
			if($AJ_TIME > $item['totime']) {
				...
				if($item['process'] == 1) {
					...
					if($item['answer'] > 1) {
						$totime = $AJ_TIME + $MOD['votedays']*86400;
						$db->query("UPDATE {$table} SET process=2,totime=$totime,updatetime='$AJ_TIME' WHERE itemid=$itemid");
					} else {
						$db->query("UPDATE {$table} SET process=0,updatetime='$AJ_TIME' WHERE itemid=$itemid");
					}
					$reload = true;
					...
					if($a && $a['vote'] > $MOD['minvote']) {
						$aid = intval($a['itemid']);
						$db->query("UPDATE {$table} SET process=3,aid=$aid,updatetime='$AJ_TIME' WHERE itemid=$itemid");
						if($a['username']) {
							if($item['credit']) {
								credit_add($a['username'], $item['credit']);
								credit_record($a['username'], $item['credit'], 'system', lang($L['record_reward'], array($MODULE[$moduleid]['name'])), 'ID:'.$itemid);
			...
			$head_title = $L['answer_question'].$AJ['seo_delimiter'].$item['title'].$AJ['seo_delimiter'].$MOD['name'];
		}
	break;
}
