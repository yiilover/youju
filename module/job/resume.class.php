...
class resume {
    ...
    function resume($moduleid) {
		global $db, $AJ_PRE, $MOD;
		$this->moduleid = $moduleid;
		$this->table = $AJ_PRE.'resume';
		$this->table_data = $AJ_PRE.'resume_data';
		$this->db = &$db;
	...
	function pass($post) {
		...
		if(intval($post['byear']) > 9999 || intval($post['byear']) < 1900 || date('Y', $AJ_TIME) - intval($post['byear']) > 100) return $this->_(lang('message->pass_resume_byear'));
	...
	function set($post) {
		global $MOD, $AJ_TIME, $AJ_IP, $TYPE, $_username, $_userid, $GENDER, $MARRIAGE, $EDUCATION;
		...
		$post['age'] = date('Y', $AJ_TIME) - intval($post['byear']);
