...
switch($action) {
	case 'add':
		if($MG['resume_limit'] && $limit_used >= $MG['resume_limit']) dalert(lang($L['info_limit'], array($MG['resume_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		if($MG['day_limit']) {
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if($_userid) {
					set_cookie('dmsg', $msg);
					$forward = $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid.'&resume=1&status='.$post['status'];
			...
			if($itemid) {
				...
				$open = 3;
				if($_userid) {
					$r = $db->get_one("SELECT * FROM {$AJ_PRE}resume a,{$AJ_PRE}resume_data c WHERE a.itemid=c.itemid AND a.username='$_username' ORDER BY a.edittime DESC");
					if($r) {
						extract($r);
						list($byear, $bmonth, $bday) = explode('-', $birthday);
					} else {
	...
	case 'apply_delete':
		$itemid or message();
		$apply = $db->get_one("SELECT * FROM {$AJ_PRE}job_apply WHERE applyid='$itemid' AND apply_username='$_username'");
		if($apply) {
			if($apply['status']>0) $db->query("UPDATE {$AJ_PRE}job SET apply=apply-1 WHERE itemid='$apply[jobid]'");
			$db->query("DELETE FROM {$AJ_PRE}job_apply WHERE applyid='$itemid'");
		}
		dmsg($L['success_delete'], $forward);
	break;
	case 'apply':
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}job_apply a LEFT JOIN {$AJ_PRE}resume r ON a.resumeid=r.itemid LEFT JOIN {$AJ_PRE}job j ON a.jobid=j.itemid WHERE a.apply_username='$_username' $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT a.*,r.title AS resumetitle,j.title,j.linkurl FROM {$AJ_PRE}job_apply a LEFT JOIN {$AJ_PRE}resume r ON a.resumeid=r.itemid LEFT JOIN {$AJ_PRE}job j ON a.jobid=j.itemid WHERE a.apply_username='$_username' $condition ORDER BY a.applyid DESC LIMIT $offset,$pagesize");
	...
	case 'refresh':
		...
		$item = $do->get_one();
		if(!$item || $item['username'] != $_username) message();
		if($MG['refresh_limit'] && $AJ_TIME - $item['edittime'] < $MG['refresh_limit']) dalert(lang($L['refresh_limit'], array($MG['refresh_limit'])), $forward);
		$do->refresh($itemid);
		dmsg($L['success_update'], $forward);
	break;
	default:
...
if($_userid) {
	$nums = array();
	for($i = 1; $i < 4; $i++) {
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}resume WHERE username='$_username' AND status=$i");
		$nums[$i] = $r['num'];
	}
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}job_apply WHERE apply_username ='$_username'");
	$nums['apply'] = $r['num'];
}
...
switch($action) {
	case 'add':
		if($MG['job_limit'] && $limit_used >= $MG['job_limit']) dalert(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		if($MG['day_limit']) {
			$today = strtotime(timetodate($AJ_TIME, 3).' 00:00:00');
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $sql AND addtime>$today");
			if($r && $r['num'] >= $MG['day_limit']) dalert(lang($L['day_limit'], array($MG['day_limit'])), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
	...
	case 'resume_show':
		$itemid or message();
		$resumeid or message();
		$db->query("UPDATE {$AJ_PRE}job_apply SET status=2 WHERE applyid='$itemid' AND job_username='$_username' AND status=1");
		dheader($MOD['linkurl'].rewrite('resume.php?itemid='.$resumeid));
	break;
	case 'resume_delete':
		$itemid or message();
		$apply = $db->get_one("SELECT * FROM {$AJ_PRE}job_apply WHERE applyid='$itemid' AND job_username='$_username' AND status>0");
		if($apply) {
			$db->query("UPDATE {$AJ_PRE}job_apply SET status=0 WHERE applyid='$itemid'");
			$db->query("UPDATE {$AJ_PRE}job SET apply=apply-1 WHERE itemid='$apply[jobid]'");
		}
		dmsg($L['success_delete'], $forward);
	break;
	case 'resume_invite':
		$itemid or message();
		$apply = $db->get_one("SELECT * FROM {$AJ_PRE}job_apply WHERE applyid='$itemid' AND job_username='$_username'");
		$apply or message($L['msg_not_apply']);
		$resume = $db->get_one("SELECT * FROM {$AJ_PRE}resume WHERE itemid='$apply[resumeid]'");
		...
		$job = $db->get_one("SELECT * FROM {$AJ_PRE}job WHERE itemid='$apply[jobid]' AND status=3");
		$job or message($L['msg_not_job']);
		if($job['totime'] && $job['totime'] < $AJ_TIME) message($L['msg_job_expired']);
		$title = lang($L['msg_invite_title'], array($job['company']));
		$joburl = linkurl($MOD['linkurl'].$job['linkurl'], 1);
		$db->query("UPDATE {$AJ_PRE}job_apply SET status=3 WHERE applyid='$itemid' AND job_username='$_username' AND status>0");
	break;
	case 'resume':
		$condition = '';
		if($keyword) $condition .= " AND r.keyword LIKE '%$keyword%'";
		if($itemid) $condition .= " AND j.itemid=$itemid";
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}job_apply a LEFT JOIN {$AJ_PRE}resume r ON a.resumeid=r.itemid LEFT JOIN {$AJ_PRE}job j ON a.jobid=j.itemid WHERE a.job_username='$_username' AND a.status>0 $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT a.*,r.truename,r.catid,r.gender,r.education,r.school,r.areaid,r.age,r.experience,j.title,j.linkurl FROM {$AJ_PRE}job_apply a LEFT JOIN {$AJ_PRE}resume r ON a.resumeid=r.itemid LEFT JOIN {$AJ_PRE}job j ON a.jobid=j.itemid WHERE a.job_username='$_username' AND a.status>0 $condition ORDER BY a.applyid DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
			$lists[] = $r;
		}
	break;
	case 'talent_delete':
		$itemid or message();
		$db->query("DELETE FROM {$AJ_PRE}job_talent WHERE username='$_username' AND talentid=$itemid");
		dmsg($L['success_delete'], $forward);
	break;
	case 'talent':
		$condition = '';
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}job_talent t LEFT JOIN {$AJ_PRE}resume r ON t.resumeid=r.itemid WHERE t.username='$_username' $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}job_talent t LEFT JOIN {$AJ_PRE}resume r ON t.resumeid=r.itemid WHERE t.username='$_username' $condition ORDER BY t.talentid DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
...
if($_userid) {
	$nums = array();
	for($i = 1; $i < 5; $i++) {
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}job WHERE username='$_username' AND status=$i");
		$nums[$i] = $r['num'];
	}
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}job_talent WHERE username='$_username'");
	$nums['talent'] = $r['num'];
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}job_apply WHERE job_username ='$_username' AND status>0");
	$nums['resume'] = $r['num'];
}
