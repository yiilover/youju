...
$item = $db->get_one("SELECT * FROM {$AJ_PRE}resume WHERE itemid=$itemid AND status=3");
...
$item = $db->get_one("SELECT * FROM {$AJ_PRE}job_talent WHERE resumeid=$itemid AND username='$_username'");
if($item) message($L['msg_talent_exist'], $linkurl);
$db->query("INSERT INTO {$AJ_PRE}job_talent (resumeid,username,jointime) VALUES ('$itemid','$_username','$AJ_TIME')");
