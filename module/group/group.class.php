...
class group {
    ...
    function group($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','areaid','level','title','style','fee','introduce','marketprice','price','savemoney','discount','minamount','amount','logistic','thumb','tag','status','hits','orders','username','totime','editor','addtime','edittime','ip','template','linkurl','filepath','note','company','truename','telephone','mobile','address','email','msn','qq','ali','skype','houseid','housename');
    }
	...
	function get_list($condition = 'status=3', $order = 'edittime DESC', $cache = '') {
		global $MOD, $pages, $page, $pagesize, $offset, $items;if($page > 1 && $sum) {
			$items = $sum;
		} else {
	...
	function refresh($itemid) {
		global $AJ_TIME;
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		$this->db->query("UPDATE {$this->table} SET edittime='$AJ_TIME' WHERE itemid IN ($itemids)");
	}
