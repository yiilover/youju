...
$EXT['archiver_enable'] or dheader(AJ_PATH);
$AJ_BOT or dheader(AJ_PATH);
$N = $M = $T = array();
...
foreach($MODULE as $k=>$v) {
	if(!$v['islink'] && $v['ismenu'] && $v['moduleid'] > 4) {
		if($k == $mid) $vmid = 1;
		$v['url'] = $AJ['rewrite'] ? rewrite('index.php?mid='.$k) : '?mid-'.$k.'.html';
		$N[] = $v;
	}
}
...
$totime = $t['maxtime'] > $AJ_TIME ? $AJ_TIME : $t['maxtime'];
...
for($i = $toyear; $i >= $fromyear; $i--) {
	for($j = ($i == $toyear ? $tomonth : 12); $j >= ($i == $fromyear ? $frommonth : 1); $j--) {
		...
		$r['month'] = $i.($j < 10 ? '0' : '').$j;
		$r['url'] = $AJ['rewrite'] ? rewrite('index.php?mid='.$mid.'&month='.$r['month']) : '?mid-'.$mid.'-month-'.$r['month'].'.html';
		$M[$r['month']] = $r;
	}
}
...
if(isset($month) && isset($M[$month])) {
	...
	$demo_url = $AJ['rewrite'] ? rewrite('index.php?mid='.$mid.'&month='.$month.'&page={aijiacms_page}') : '?mid-'.$mid.'-month-'.$month.'-page-{aijiacms_page}.html';
