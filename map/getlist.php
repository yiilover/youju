...
$result = mysql_query("SELECT * FROM $hsall where $condition $order LIMIT $offset,$pagesize ");
	...
	while($r = mysql_fetch_array($result)) {
		...
		$thumb=imgurl($r[thumb], 1);
		$r['title']=charsetIconv(dsubstr($r[title], 38, ''));
   ...
   <div class="list_item" title="<?php echo $r['title'];?>"><ul><li class="lit_1"><a href="<?php echo $r['linkurl'];?>" target="_blank"><img src="<?php echo $thumb;?>" height="48" width="64"></a></li><li class="lit_2"><a href="<?php echo $r['linkurl'];?>" target="_blank"><?php echo $r['title'];?></a><br><?php echo $r['room'];?>室<?php echo $r['hall'];?>厅 , <?php echo $r['houseearm'];?>㎡ , <?php echo $r['floor1'];?>层共<?php echo $r['floor3'];?>层<br><span><a href="<?php echo userurl($r[username], '');?>" target="_blank"><?php echo charsetIconv($r['truename']);?></a> <?php echo $edittime;?> 发布</span></li><li class="lit_3"><?php echo $price;?><br><i><?php echo $r['danjia'];?></i></li></ul></div>
