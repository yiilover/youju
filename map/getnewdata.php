...
//echo  $condit'ion;
 ...
 $str='[';
	...
	while($r = mysql_fetch_array($result)) {
		...
		$cnum=$r['num'];
		$typeid=$r['typeid'];
		if ($typeid==0){ $typename='待售';}
		if ($typeid==1){ $typename='在售';}
		if ($typeid==2){ $typename='尾盘';}
		if ($typeid==3){ $typename='售完';}
		$thunb=imgurl($r[thumb], 1);
	...
	$map=explode(",",$arr);
		foreach($map as $key =>$value){
		  $x =$map['0'];
		   ...
		   $a='{cid:'.$r['itemid'].','.'cname:"'.$r['title'].'",'.'cnum:"'.$cnum.'",'.'averprice:"'.$r['price'].'",'.'imgsrc:"'.$thunb.'",'.'href:"'.$r['linkurl'].'",'.'protype:"'.$catname.'",'.'emplor:"'.$r['kfs'].'",'.'tel:"'.$r['telephone'].'",'.'hseat:"'.$r['address'].'",'.'saletype:"'.$typename.'",'.'sales:"'.$r['typeid'].'", x:'.$x.', y:'.$y.', xs:'.$y.'},';
