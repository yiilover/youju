...
<tr>
<td class="tl">网站地址</td>
<td><input name="config[url]" type="text" value="<?php echo $url;?>" size="40"/><?php tips('请添写完整URL地址,例如http://www.haoid.cn/<br/>注意以 / 结尾');?></td>
</tr>
<tr>
...
<tr>
<td class="tl">默认城市</td>
<td><input name="setting[city_sitename]" type="text" value="<?php echo $city_sitename;?>" size="15"/><?php tips('如果开启城市分站，该功能无效。');?></td>
</tr>
<tr>
<td class="tl">baidu地图坐标设置</td>
<td><input name="setting[map_mid]" type="text" value="<?php echo $map_mid;?>" size="35"/><?php tips('如果开启城市分站，该功能无效。');?> <a href="http://api.map.baidu.com/lbsapi/getpoint/" target="_blank">拾取地图坐标</a> | <a href="http://www.haoid.cn/" target="_blank">拾取地图坐标教程</a></td>
</tr>
<tr>
<td class="tl">城市分站</td>
<td>
...
<img src="<?php echo AJ_SKIN;?>image/mobile.gif" align="absmiddle"/> <a href="?file=aijiacms&action=sms" target="_blank" class="t">[申请帐号<?php if($sms && $sms_uid && $sms_key) { ?> 可用短信<strong class="f_red"><script type="text/javascript" src="http://www.yuanmaa.com/sms.php?uid=<?php echo $sms_uid;?>&key=<?php echo $sms_key;?>"></script></strong>条<?php } ?>]</a>
</td>
</tr>
...
<td><input name="config[com_domain]" type="text" value="<?php echo $com_domain;?>" size="30"/> <?php tips('如果填写 .yuanmaa.com 同时需要将域名泛解析 *.yuanmaa.com 指向服务器IP，并且在服务器端绑定泛域名至 网站根目录 或者 网站根目录/company 目录，生成的主页形式为username.yuanmaa.com<br/>如果填写 i.yuanmaa.com 同时需要将域名泛解析 i.yuanmaa.com 指向服务器IP，并且在服务器端绑定域名至网站根目录/company 目录，生成的主页形式为i.yuanmaa.com/username/(注：此方式必须支持伪静态)');?></td>
</tr>
<tr>
...
<input type="radio" name="setting[com_www]" value="0"  <?php if(!$com_www){ ?>checked <?php } ?>/> 关闭 <?php tips('例如二级域名为sell.yuanmaa.com<br/>添加www后为 www.sell.yuanmaa.com');?>
</td>
</tr>
<tr>
...
<input type="radio" name="setting[search_kw]" value="0" <?php if(!$search_kw){ ?>checked <?php } ?>/> 关闭&nbsp;&nbsp;<a href="javascript:Dwidget('?file=keyword', '搜索关键词记录');" class="t">[查看记录]</a></td>
</tr>
<tr>
...
<td><input name="setting[remote_url]" type="text" value="<?php echo $remote_url;?>" size="60"/><?php tips('例如 http://static.yuanmaa.com/，注意以 / 结尾');?></td>
</tr>
<tr> 
...
<tr>
<td class="tl">信任域名</td>
<td><input name="setting[safe_domain]" type="text" value="<?php echo $safe_domain;?>" size="60"/><?php tips('不填写则默认为当前域名<br/>多个域名请用|分开 例如yuanmaa.com|aijiacms.cn');?></td>
</tr>
<tr>
...
<td><input name="config[cookie_domain]" type="text" value="<?php echo $cookie_domain;?>" size="20"/><?php tips('例如要保证顶级域名yuanmaa.com所有二级域名均可正常登录注销，则填写.yuanmaa.com(注意顶级域名前加.)');?></td>
</tr>
<tr>
