	...
	case 'edit':
		if($submit) {
			...
			if($areaid) {
				...
				$areaid = $listorder = 0;
				$name = $style = $letter = $domain = $iparea = $template = $seo_title = $seo_keywords = $seo_description= $hits = $hits= $map_mid = $map_mid = '';
			}
...
class city {
	...
	function get_list($condition) {
		...
		while($r = $this->db->fetch_array($result)) {
			$r['linkurl'] = AJ_PATH.'city.php?action=go&forward=&areaid='.$r['areaid'];
			$lists[] = $r;
		}
		return $lists;
	}
