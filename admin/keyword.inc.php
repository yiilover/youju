...
switch($action) {
	case 'letter':
		if(!$word) exit('');
		if(strtoupper(AJ_CHARSET) != 'UTF-8') $word = convert($word, 'UTF-8', AJ_CHARSET);
		exit(gb2py($word));
	break;
	default:
		if($submit) {
...
class keyword {
	...
	function add($post) {
		...
		$this->db->query("INSERT INTO {$this->table} (moduleid,word,keyword,letter,items,total_search,month_search,week_search,today_search,updatetime,status) VALUES('$post[moduleid]','$post[word]','$post[keyword]','$post[letter]','$post[items]','$post[total_search]','$post[month_search]','$post[week_search]','$post[today_search]','$AJ_TIME', '$post[status]')");
	}
