...
switch($action) {
	case 'download':
		...
		$url = 'http://www.aijiacms.com/update.php?product=b2b&release='.$release.'&charset='.AJ_CHARSET;
	...
	case 'copy':
		...
		foreach($files as $v) {
			...
			file_copy($v, $file_a) or msg('无法覆盖'.str_replace(AJ_ROOT.'/', '', $file_a).'<br/>请设置此文件及上级目录属性为可写，然后刷新此页');
		}
