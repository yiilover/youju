...
if($swfupload) {//Fix FlashPlayer Bug
	...
	if($swf_userid != $_userid && is_md5($swf_auth)) {
		...
		if($swf_auth = md5($swf_userid.$swf_username.$swf_groupid.$swf_company.AJ_KEY.$AJ_IP)) {
			$_userid = $swf_userid;
...
if($do->save()) {
	...
	$t = @getimagesize(AJ_ROOT.'/'.$do->saveto);
	...
	if($do->image) {
		...
		$info = getimagesize(AJ_ROOT.'/'.$do->saveto);
		$img_w = $info[0];
		$img_h = $info[1];
		...
		if($from == 'thumb') {
			...
			if($_groupid == 1 && !isset($watermark)) $AJ['water_type'] = 0;
			if($AJ['water_type']) {
				$image = new image(AJ_ROOT.'/'.$do->saveto);
				if($AJ['water_type'] == 2) {
					$image->waterimage();
		...
		if($ftp->connected) {
			...
			if($ftp->dftp_put($do->saveto, $remote)) {
				...
				if(strpos($do->saveto, '.thumb.') !== false) {
						$local = str_replace(''.$do->ext, '', $do->saveto);
					$remote = str_replace(''.$do->ext, '', $exp[1]);
					$ftp->dftp_put($local, $remote);
					file_del(AJ_ROOT.'/'.$local);
						$local = str_replace(''.$do->ext, '.middle.'.$do->ext, $do->saveto);
					$remote = str_replace(''.$do->ext, '.middle.'.$do->ext, $exp[1]);
					$ftp->dftp_put($local, $remote);
					file_del(AJ_ROOT.'/'.$local);
				}
			}
