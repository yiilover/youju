...
function keyword($kw, $items, $moduleid) {
	...
	if($r) {
		...
		$db->query("UPDATE {$db->pre}keyword SET items='$items',updatetime='$AJ_TIME',total_search=total_search+1,month_search=$month_search,week_search=$week_search,today_search=$today_search WHERE itemid=$r[itemid]");
	} else {
		$letter = gb2py($kw);
		$status = $AJ['search_check_kw'] ? 2 : 3;
		$db->query("INSERT INTO {$db->pre}keyword (moduleid,word,keyword,letter,items,updatetime,total_search,month_search,week_search,today_search,status) VALUES ('$moduleid','$kw','$kw','$letter','$items','$AJ_TIME','1','1','1','1','$status')");
	}
}
...
function money_record($username, $amount, $bank, $editor, $reason, $note = '') {
	...
	if($username && $amount) {
		...
		$balance = $r['money'];
		$db->query("INSERT INTO {$db->pre}finance_record (username,bank,amount,balance,addtime,reason,note,editor) VALUES ('$username','$bank','$amount','$balance','$AJ_TIME','$reason','$note','$editor')");
	}
}
...
function sms_record($username, $amount, $editor, $reason, $note = '') {
	...
	if($username && $amount) {
		...
		$balance = $r['sms'];
		$db->query("INSERT INTO {$db->pre}finance_sms (username,amount,balance,addtime,reason,note,editor) VALUES ('$username','$amount','$balance','$AJ_TIME','$reason','$note','$editor')");
	}
}
...
function check_pay($moduleid, $itemid) {
	global $db, $_username, $AJ_TIME, $MOD;
...
function crypt_sign($string) {
	global $AJ_IP;
	return strtoupper(md5(md5($AJ_IP.$string.AJ_KEY)));
}
