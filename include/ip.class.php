...
class ip {
	...
	function area() {
		if($this->iptype) {
			$func = $this->iptype;
			$this->ipfile = AJ_ROOT.'/file/ipdata/'.$func.'.dat';
			return $this->$func();
		}
		foreach(array('wry', 'tiny', 'sina', 'youdao') as $d) {
			$ipfile = AJ_ROOT.'/file/ipdata/'.$d.'.dat';
			if(is_file($ipfile)) {
				$this->ipfile = $ipfile;
				return $this->$d();
			}
	...
	function wry() {
		...
		return convert($ipaddr, 'GBK', AJ_CHARSET);
	}
	...
	function tiny() {
		...
		if($index_length['len']) return convert(fread($fp, $index_length['len']), 'GBK', AJ_CHARSET);
		return 'Unknown';
	}
	...
	function sina() {
		...
		if($data && strpos($data, 'remote_ip_info') !== false) {
			...
			return $area ? convert($area, 'UTF-8', AJ_CHARSET) : 'Unknown';
		}
	...
	function youdao() {
		...
		if($data && strpos($data, '<location>') !== false) {
			...
			$area = $t2[0];
			return $area ? convert($area, 'GBK', AJ_CHARSET) : 'Unknown';
		}
