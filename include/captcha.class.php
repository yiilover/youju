...
class captcha {
	...
	function question($id) {
		...
		$_SESSION['answerstr'] = md5(md5($r['answer'].AJ_KEY.$this->ip));
	...
	function image() {
		...
		$string = $this->mk_str();
		$_SESSION['captchastr'] = md5(md5(strtoupper($string).AJ_KEY.$this->ip));
		...
		if($this->cn) {
			$string = convert($string, AJ_CHARSET, 'UTF-8');
			$angle = mt_rand(-10, 10);
			$size = mt_rand(13, 20);
			$font = $this->font;
			$X = $size + mt_rand(5, 10);
	...
	function mk_str() {
		$str = '';
		if($this->cn) {
			$step = strtoupper(AJ_CHARSET) == 'UTF-8' ? 3 : 2;
