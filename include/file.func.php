...
function dir_create($path) {
	...
	if(AJ_CACHE != AJ_ROOT.'/file/cache' && strpos($path, AJ_CACHE) !== false) {
		...
		for($i = 0; $i < $max; $i++) {
			...
			if(!is_file($cur_dir.'/index.html') && !is_file($cur_dir.'/index.php')) file_copy(AJ_ROOT.'/file/index.html', $cur_dir.'/index.html');
		}
	} else {
		...
		for($i = 0; $i < $max; $i++) {
			...
			@mkdir($cur_dir);
			if(AJ_CHMOD) @chmod($cur_dir, AJ_CHMOD);
			if($idx && !is_file($cur_dir.'/index.html') && !is_file($cur_dir.'/index.php')) file_copy(AJ_ROOT.'/file/index.html', $cur_dir.'/index.html');
		}
	}
	return is_dir($path);
}
...
function dir_delete($dir) {
	...
	$dirs = array(AJ_ROOT.'/admin/', AJ_ROOT.'/api/', AJ_CACHE.'/', AJ_ROOT.'/file/', AJ_ROOT.'/include/', AJ_ROOT.'/lang/', AJ_ROOT.'/member/', AJ_ROOT.'/module/', AJ_ROOT.'/extend/', AJ_ROOT.'/skin/', AJ_ROOT.'/template/', AJ_ROOT.'/wap/');
