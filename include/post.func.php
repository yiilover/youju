...
function deditor($moduleid = 1, $textareaid = 'content', $toolbarset = 'Default', $width = 500, $height = 400) {
	...
	$moddir = defined('AJ_ADMIN') ? $MODULE[2]['moduledir'].'/' : '';
	$editor = '';
	...
	$editor .= '}';
	$editor .= '</script>';
	$save = $textareaid == 'content' && $_userid && $AJ['save_draft'];
	if($AJ['save_draft'] == 2 && !defined('AJ_ADMIN')) $save = false;
	$editor .= '<script type="text/javascript" src="'.AJ_STATIC.'file/script/fckeditor.js"></script>';
	if($save) $editor .= '<script type="text/javascript" src="'.AJ_STATIC.'file/script/draft.js"></script>';
	echo $editor;
}
...
function dstyle($name, $value = '') {
	...
	$style .= '<input type="hidden" name="'.$name.'" id="color_input_'.$aijiacms_style_id.'" value="'.$color.'"/><img src="'.AJ_PATH.'file/image/color.gif" width="21" height="18" align="absmiddle" id="color_img_'.$aijiacms_style_id.'" style="cursor:pointer;background:'.$color.'" onclick="color_show('.$aijiacms_style_id.', Dd(\'color_input_'.$aijiacms_style_id.'\').value, this);"/>';
	return $style;
}
...
function dcalendar($name, $value = '', $sep = '-') {
	...
	$calendar .= '<input type="text" name="'.$name.'" id="'.$id.'" value="'.$value.'" size="10" onfocus="ca_show(\''.$id.'\', this, \''.$sep.'\');" readonly ondblclick="this.value=\'\';"/> <img src="'.AJ_STATIC.'file/image/calendar.gif" align="absmiddle" onclick="ca_show(\''.$id.'\', this, \''.$sep.'\');" style="cursor:pointer;"/>';
	return $calendar;
}
...
function gb2py($text, $exp = '') {
	if(!$text) return '';
	if(strtolower(AJ_CHARSET) != 'gbk') $text = convert($text, AJ_CHARSET, 'gbk');
	$data = array();
...
function _clear_link($matchs) {
	if(strpos($matchs[0], AJ_PATH) !== false) return $matchs[0];
	if(AJ_DOMAIN && strpos($matchs[0], AJ_DOMAIN) !== false) return $matchs[0];
	return $matchs[1];
}
...
function save_remote($content, $ext = 'jpg|jpeg|gif|png|bmp', $self = 0) {
	...
	foreach($matches[2] as $k=>$url) {
		...
		$filename = timetodate($AJ_TIME, 'H-i-s').'-'.rand(10, 99).'-'.$_userid.'.'.$file_ext;
...
function delete_upload($file, $userid) {
	...
	if($AJ['uploadlog']) $db->query("DELETE FROM {$db->pre}upload_".($userid%10)." WHERE item='".md5($fileurl)."'");
}

function clear_upload($content = '', $itemid = 0) {
	global $CFG, $AJ, $db, $session, $_userid;
	...
	foreach($_SESSION['uploads'] as $file) {
		if(strpos($content, $file) === false) {
			...
			if($AJ['uploadlog'] && $itemid) $update[] = "'".md5($file)."'";
		}
	}
...
function sync_weibo($site, $moduleid, $itemid) {
	...
	return 'document.write(\'<img src="'.AJ_PATH.'api/oauth/'.$file.'?auth='.encrypt($moduleid.'-'.$itemid).'" width="1" height="1"/>\');';
}
/*
		读取新房分类
	*/
