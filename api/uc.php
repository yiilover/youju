...
//cache_write('uc'.date('Y-m-d-H-i-s', $AJ_TIME).'.php', $uc_arr);
...
switch($action) {
	...
	case 'synlogin':
		...
		$aijiacms_auth = encrypt($user['userid']."\t".$user['username']."\t".$user['groupid']."\t".$user['password']."\t".$user['admin'], md5(AJ_KEY.$_SERVER['HTTP_USER_AGENT']));
		ob_clean() ;
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
		set_cookie('auth', $aijiacms_auth, $cookietime);
		$db->query("UPDATE {$AJ_PRE}member SET loginip='$AJ_IP',logintime=$AJ_TIME,logintimes=logintimes+1 WHERE userid=$userid");
		exit('1');
	break;
	case 'synlogout':
		if($_userid) {
