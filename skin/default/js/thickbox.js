...
function TB_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
	...
	try {
		...
		$("body").append("<div id='TB_load'><img src='/data/loadingAnimation.gif' /></div>");
		TB_load_position();
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
			...
			$("#TB_window").append("<a href='' id='TB_ImageOff' title='Close'><img id='TB_Image' src='"+url+"' width='"+imageWidth+"' height='"+imageHeight+"' alt='"+caption+"'/></a>" + "<div id='TB_caption'>"+caption+"<div id='TB_secondLine'>" + TB_imageCount + TB_PrevHTML + TB_NextHTML + "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='Close or Escape Button'>关闭</a></div>"); 		
		...
		if(urlType=='.htm'||urlType=='.html'||urlType=='.php'||urlType=='.asp'||urlType=='.aspx'||urlType=='.jsp'||urlType=='.jst'||urlType=='.rb'||urlType=='.txt'||urlType=='.cfm' || (url.indexOf('TB_inline') != -1) || (url.indexOf('TB_iframe') != -1) ){//code to show html pages
			...
			if(url.indexOf('TB_iframe') != -1){				
					...
					if(urlNoQuery.indexOf("@") != -1){ 
						...
						lykUrl_1 = urlNoQuery.substr(TB_strpos(urlNoQuery,"@")+1);
						lykUrl_2 = urlNoQuery.substr(0,TB_strpos(urlNoQuery,"@"));
						urlNoQuery = lykUrl_2 + "?" + lykUrl_1;
					}
